...
def upProfile(num,value):
    if(num== 1):
        ...
        # frame3.place(x=100, y=50, width=1150, height=600)

        # Label(frame3, text="Fill the box with Updated value if none keep empty", font=("arial", 14, "bold"), fg="white",
        #       bg="black").place(x=50, y=50)
        # Label(frame3, text="Name", font=("arial", 14, "bold"), fg="white", bg="black").place(x=50, y=90)
        # Label(frame3, text="Password", font=("arial", 14, "bold"), fg="white", bg="black").place(x=50, y=130)
        # Label(frame3, text="Email", font=("arial", 14, "bold"), fg="white", bg="black").place(x=50, y=170)
        # upName = ttk.Entry(frame3, font=("arial", 15, "bold"))
        # upName.place(x=250, y=90)
        # upPass = ttk.Entry(frame3, font=("arial", 15, "bold"))
        # upPass.place(x=250, y=130)
        # upEmail = ttk.Entry(frame3, font=("arial", 15, "bold"))
        # upEmail.place(x=250, y=170)
...
def manageEvent(val):
    ...
    # frame1.place(x=0,y=0,width=510,height=320)

    # Label(frame1, text="Event Name").place(x=50,y=90)
    # Label(frame1, text="Event Id").place(x=50,y=130)
    # Label(frame1, text="City").place(x=50,y=170)
    # Label(frame1, text="Date").place(x=50,y=210)
    # eventname = Entry(frame1)
    # eventname.place(x=250,y=90)
    # eid = IntVar(frame1, id(eventname))
    # eventid = Entry(frame1, textvariable=eid)
    # eventid.place(x=250,y=130)
    # city = Entry(frame1)
    # city.place(x=250,y=170)
    # date = Entry(frame1)
    # date.place(x=250,y=210)
    ...
    # ttk.Separator(frame1, orient='horizontal').place(y=310, relwidth=1)
    # tree = ttk.Treeview(frame1, columns=('eventId','eventName','eventDate', 'empId'), show='headings')
    ...
    # sql= "select eventId,eventName,eventDate, empId from EventManage"
    # mycur.execute(sql)
    # record=mycur.fetchall()
    # for i in record:
    ...
    #scrollbar = ttk.Scrollbar(newevent, orient=VERTICAL, command=tree.yview)
    #tree.configure(yscroll=scrollbar.set)
    #scrollbar.grid(row=9)
    ...
    def subevent():
        ...
        mydb.commit()
        messagebox.showinfo("Event", "Yippee\nEvent Added successfully")
        newevent.destroy()
    ...
    # Button(frame1, text="Submit", command=subevent).place(x=50,y=260)
    # Button(frame1, text="Cancel", command=canEvent).place(x=250,y=260)
...
def opencust(val):
    ...
    # Label(frame2, text="Welcome", font=("arial",30,"bold"), fg="red",bg="black").place(x=450, y=25)
    mycur1 = mydb.cursor()
    ...
    # Label(frame2, text=dres, fg="white", bg="black", font=("arial", 20, "bold")).place(x=410, y=80)
    # custbtn = Button(frame2, text="Manage Event", command=lambda: manageEvent(val), font=("arial", 12, "bold"),
    #                  relief=RIDGE, fg="white", bg="blue")
    # custbtn.place(x=430, y=140)
    # updbtn = Button(frame2, text="Update Profile", command=lambda: upProfile(2, val), font=("arial", 12, "bold"),
    #                 relief=RIDGE, fg="white", bg="blue")
    # updbtn.place(x=630, y=140)
